      ...
      <div className="glass-card bg-[#0c1324] border border-[#424754] rounded-[2rem] max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative my-8">
        ...
        <div className="border-b border-[#424754] pb-6 mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-white">ELECTRICITY UTILITY STATEMENT</h2>
              <p className="text-[11px] font-mono text-[#adc6ff]">
                Document ID: {invoice.id.toUpperCase()} • Period: {invoice.period}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 bg-[#151b2d] rounded-2xl border border-[#424754] mb-6 font-mono text-xs">
          <div>
            <span className="text-[10px] text-[#94a3b8] uppercase block">Bill Date</span>
            <span className="font-bold text-white">{invoice.bill_date}</span>
          </div>
          <div>
            <span className="text-[10px] text-[#94a3b8] uppercase block">Due Date</span>
            <span className="font-bold text-[#adc6ff]">{invoice.due_date}</span>
          </div>
          <div>
            <span className="text-[10px] text-[#94a3b8] uppercase block">Meter No</span>
            <span className="font-bold text-white">{account.meterNumber}</span>
          </div>
          <div>
            <span className="text-[10px] text-[#94a3b8] uppercase block">Tariff</span>
        ...
        <div className="space-y-3 mb-6">
          <h3 className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider">Itemized Breakdown</h3>
          
          <div className="space-y-2 font-mono text-xs">
            <div className="flex justify-between py-2 border-b border-[#424754]/50">
              <span className="text-[#dce1fb]">
                Active Energy ({invoice.kwh.toLocaleString('ru-RU')} kWh @ {invoice.unit_rate_tl.toFixed(4)} TL)
              </span>
              <span className="font-bold text-white">{net.toLocaleString('ru-RU')} TL</span>
            </div>

            <div className="flex justify-between py-2 border-b border-[#424754]/50">
              <span className="text-[#94a3b8]">Daily Consumption Average</span>
              <span className="text-white">{invoice.daily_avg_kwh} kWh/day</span>
            </div>

            <div className="flex justify-between py-2 border-b border-[#424754]/50">
              <span className="text-[#94a3b8]">Distribution & Municipal Taxes (12%)</span>
          ...
          <div className="flex items-center gap-2">
            {invoice.status === 'PAID' ? (
              <span className="px-3 py-1.5 rounded-full bg-[#4edea3]/15 text-[#4edea3] text-xs font-bold flex items-center gap-1.5 border border-[#4edea3]/30">
                <CheckCircle className="w-4 h-4" /> Paid Statement
              </span>
            ) : (
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            ...
            {invoice.status === 'PENDING' && (
              ...
              >
                ...
                <span>Pay {invoice.total_amount_tl.toLocaleString('ru-RU')} TL</span>
              </button>
            )}
          </div>
        </div>
